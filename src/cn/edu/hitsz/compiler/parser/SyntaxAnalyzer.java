...
public class SyntaxAnalyzer {
    ...
    private final List<Token> myTokens = new ArrayList<>();
    ...
    public void run() {
        ...
        Status currentStatus = lrTable.getInit();
        ...
        for (int j = 0; j < myTokens.size(); j++) {
            Token myToken = myTokens.get(j);
            currentStatus = statusStack.peek();
            action = lrTable.getAction(currentStatus, myToken);
            switch (action.getKind()) {
                case Shift -> {
                    callWhenInShift(currentStatus, myToken);
